...
Formdaki "İmza tipi" açılır listesi `CAdES-BES` / `CAdES-T` / `XAdES-BES` /
`XAdES-T` seçtirir ve create isteğine `signatureProfile` olarak geçer (varsayılan
`WSIGN_SIGNATURE_PROFILE`, vsy `CAdES-BES`). `-T` (zaman damgalı) için entegratörde
