...
Sunucu protokolünün tamamını doğrular: `create → info → prepare → (yerel imza) → complete → callback`.
...
**Kapsar:** API sözleşmesi, auth, deferred prepare, CMS montajı, HMAC callback teslimi.
...
4. Tarayıcı `successRedirectUrl`'e döner; entegratör `callbackUrl`'de imzalı CMS'i alır (HMAC + nonce doğrula).
